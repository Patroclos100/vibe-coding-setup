---
description: Run and record quality gates for the active increment or release
agent: qa-supervisor-agent
---
...
5. Invoke the agents required for `release.ai`
...
- command name: /quality-run-gates
- purpose: Run and record quality gates for the active increment or release
- trigger: explicit operator request or pipeline continuation authorized by policy
- workflow invoked: release.ai
- agents involved: qa-supervisor-agent, compliance-agent, system-architect-agent
- expected artifacts: quality gate record, approval state
